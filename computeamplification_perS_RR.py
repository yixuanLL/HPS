...
def numericalanalysis(n, epsorig, delta, num_iterations, step, upperbound, clip_bound=None):
    '''
...
def probEoN(ei_arr, e1_idx):  
    ...
    for i in range(len(ei_arr)):
        ...
        pij = np.minimum((1+np.e**ej)/(np.e**ej*(1+np.e**ei)), (1+np.e**ei)/(np.e**ei*(1+np.e**ej)) )  /n #Random response
