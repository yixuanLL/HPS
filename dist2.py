...
def compute_pij(ej, dj, mech, C):
    ...
    plt.plot(x,stats.norm.pdf(x,loc=mu_i, scale = sigma_1))#, label='$R(x_1^1)$')
    x1 = (mu_i+mu_j)/2
    y = np.linspace(0,0.6,300)

    xp = np.linspace(-3, x1,300)
    plt.fill_between(xp, stats.norm.pdf(xp, loc=mu_i, scale = sigma_1), xp*0, color='g', alpha=0.3)#, label="$R(x_1^0)$ max")
    xp = np.linspace(x1, 3, 300)
    plt.fill_between(xp, stats.norm.pdf(xp, loc=mu_j, scale = sigma_1), xp*0, color='orange', alpha=0.3)#, label="$R(x_1^1)$ max")
