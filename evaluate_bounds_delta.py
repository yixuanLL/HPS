...
def plot_panel(xs, bounds):
    ...
    for b in bounds:
        ...
        for dist in ['Uniform2', 'Gauss2']: #, 'MixGauss']:
            print('dist:', dist)
            ys = list()
            for x in xs:
                eps0 = gen_eps(l, r, x, dist)
                if b.get_name() == "FV":
                    eps0 = (eps0[0], np.array([delta_l]*x))
...
def gen_eps(l, r, n, dist):
    if dist == 'Uniform2':
        ...
        delta0 = np.random.uniform(delta_l, delta_r, n)
        return (eps0, delta0)
    elif dist == 'Gauss2':
        ...
        # delta0 = np.array([delta_l]*n)
        delta0 = np.random.normal(delta_l*10, 0.5, n)
        delta0 = np.maximum(eps0, delta_l)
        delta0 = np.minimum(eps0, delta_r)        
        return (eps0, delta0)
    elif dist == 'MixGauss':
        step = int(n*0.5)
...
for bounds in bound_list: 
    ...
    # plt.yscale('log')
    plt.show()
    path = './epsilon2_delta_'+ path_name + '.pdf'
    plt.savefig(path)
    print('----'+path+'----')
    plt.close()
